    ...
    <div className="min-h-[100dvh] bg-black text-foreground selection:bg-primary selection:text-primary-foreground overflow-x-hidden">
      ...
      <main className="pt-32 md:pt-48 pb-24 px-6">
        <div className="container mx-auto max-w-7xl">
          ...
          {featured && (
            ...
            >
              <Link
                ...
                className="group grid grid-cols-1 lg:grid-cols-12 gap-12 items-center bg-[#111] border border-white/10 overflow-hidden hover:border-white/30 transition-colors"
              ...
              >
                <div className="lg:col-span-5 p-12 order-2 lg:order-1">
                  ...
                  <h2 className="text-4xl md:text-5xl font-display font-bold tracking-tight mb-6 uppercase">{featured.title}</h2>
                  <p className="text-lg text-white/70 mb-10 leading-relaxed">{featured.summary}</p>
                  <span 
                    className="inline-flex items-center gap-3 px-6 py-3 bg-white text-black font-medium group-hover:bg-white/90 transition-colors"
          ...
          >
            {rest.map((project, i) => (
              <motion.div key={project.slug} variants={fadeIn} className={project.size}>
                <Link
                  href={`/portfolio/${project.slug}`}
                  className={`group relative bg-[#111] border border-white/10 overflow-hidden flex flex-col justify-end p-8 hover:border-white/30 transition-colors h-full w-full`}
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <h3 className="text-3xl font-display font-bold tracking-tight mb-2 group-hover:text-white transition-colors">{project.title}</h3>
                    <p className="text-white/60">{project.tagline}</p>
                    <span className="inline-flex items-center gap-2 mt-6 text-xs font-mono uppercase tracking-widest text-white/40 group-hover:text-white transition-colors">
                      View case <ArrowRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" />
                    </span>
                  </div>
                </Link>
              </motion.div>
